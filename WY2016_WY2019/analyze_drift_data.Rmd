...
```{r}
driftBugs <- read.csv("clean_data/clean_drift_bugs.csv")
driftBugsComplete <- read.csv("clean_data/clean_drift_bugs_complete.csv")%>%
  mutate(Date = lubridate::ymd(Date))# zeroes filled in
driftDailyCPUE <- read_csv("clean_data/clean_drift_daily_cpue.csv")

driftFamilyAbund <-read.csv("clean_data/clean_drift_family_abundant.csv")%>%
  mutate(Date = lubridate::ymd(Date))
driftOrderAbund<- read.csv("clean_data/clean_drift_order_abundant.csv")%>%
  mutate(Date = lubridate::ymd(Date)) %>%
  ungroup()
driftOrderIEP <- read.csv("clean_data/clean_drift_order_iep.csv")
driftInund <- read.csv("clean_data/clean_drift_inund_sumCPUE.csv")
inundation <- read.csv("Data/Yolo_Bypass_Inundation1998-2021.csv") 
inundation <- inundation %>%
  mutate(Date = lubridate::ymd(inundation$Date))%>%
  mutate(Year = year(Date),
         Month = month(Date),
         WY = ifelse(month(Date) > 9, Year + 1, Year))
inundation2 <- read.csv("clean_data/inundation2.csv") %>%
  mutate(Date = lubridate::ymd(Date))%>%
  mutate(Year = year(Date),
         Month = month(Date),
         WY = ifelse(month(Date) > 9, Year + 1, Year)) %>%
  mutate(Inundation_mean = replace(Inundation_mean, Inundation_mean == 50, 25)) %>%
