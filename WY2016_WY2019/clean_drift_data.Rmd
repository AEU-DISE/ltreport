...
```{r}
drift0 <- read.csv("Data/Drift_data_20210420.csv")
taxonomy <- read.csv("Data/DriftTaxonomy.csv")
inundation <- read.csv("Data/Yolo_Bypass_Inundation1998-2021.csv")
```
...
Add inundation
```{r}
inundation2 <- select(inundation, -c(StageHeight_FW:Overtopping_LIS)) %>%
  mutate(Inundation2 = ifelse(lead(Inundation, 10) == "TRUE", "TRUE", Inundation),
         Inundation_n = ifelse(Inundation2 == "TRUE", 150, 0),
         Inundation_mean = ifelse(Inundation2 == "TRUE", 50, 0),
         Inundation_log = ifelse(Inundation2 == "TRUE", 5, 0)) %>%
  filter(WY>2015 & WY <2021)
...
Write data
```{r}
write.csv(driftBugs, "clean_data/clean_drift_bugs.csv", row.names = FALSE)
write.csv(driftBugsComplete, "clean_data/clean_drift_bugs_complete.csv", row.names = FALSE)
write.csv(driftDailyCPUE, "clean_data/clean_drift_daily_cpue.csv", row.names = FALSE)
write.csv(driftFamilyAbund, "clean_data/clean_drift_family_abundant.csv", row.names = FALSE)
write.csv(driftOrderAbund, "clean_data/clean_drift_order_abundant.csv", row.names = FALSE)
write.csv(driftOrderIEP, "clean_data/clean_drift_order_iep.csv", row.names = FALSE)

write.csv(driftInund, "clean_data/clean_drift_inund_sumCPUE.csv", row.names = FALSE)
write.csv(inundation2, "clean_data/inundation2.csv", row.names = FALSE)
