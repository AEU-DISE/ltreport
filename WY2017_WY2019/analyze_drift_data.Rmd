...
```{r}
write.csv(meanDriftBugs, here("WY2017_WY2019", "data_figures", "meanDriftBugs.csv"), row.names = FALSE)
```
...
driftOrderPercent <- driftBugs %>%
  mutate(Order2 = ifelse(is.na(Order), TaxonName, Order)) %>%
  group_by(Station, fWY) %>%
  mutate(totalCPUE = sum(CPUE)) %>%
  ungroup() %>%
  group_by(Station, fWY, totalCPUE, Order2) %>%
...
driftPercentFilt <- driftOrderPercent %>%
  mutate(Order3 = ifelse(propCPUE>=3, Order2, "Other")) %>%
  group_by(Station, fWY, Order3) %>%
  summarize(Percent = sum(propCPUE))

```

Write for combined plot
```{r}
write.csv(driftPercentFilt, here("WY2017_WY2019", "data_figures", "ProportionalData.csv"), row.names = FALSE)
```
...
inundation2$h175 = 175
colors <- data.frame(Inundation = c("Inundation", "No Inundation"), Color = c("light blue", "white"))
```

Figure 4. Inundation vs CPUE by station
need to fix this to add inundation legend and line colors/types
Export

```{r}
write.csv(driftDailyCPUE, here("WY2017_WY2019", "data_figures", "driftDailyCPUE.csv"), row.names = FALSE)
write.csv(inundation2, here("WY2017_WY2019", "data_figures", "inundation2.csv"), row.names = FALSE)
```

```{r}
# Total CPUE
(inunDriftAll <- ggplot() +
  geom_col(data = inundation2, aes(x = Date, y = h175, colour = color, fill = color)) +
  geom_line(data = driftDailyCPUE, aes(x = Date, y = CPUEDaily, linetype = Station)) +
  scale_x_date(date_breaks = "3 months", date_labels = "%b-%Y") +
   #scale_colour_manual(values = c("black", "purple4")) + 
