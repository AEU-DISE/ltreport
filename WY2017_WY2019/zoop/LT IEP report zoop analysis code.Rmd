...
zoop150.sum1 <- zoop150 %>% 
  group_by(Date, WY) %>% summarize(daily.CPUE = sum(CPUE))
...
inun.zoop.all <- ggplot() +
  ...
  geom_line(data = zoop150.sum1, aes(x = Date, y = daily.CPUE)) +
  scale_x_date(date_breaks = "3 months", date_labels = "%b-%Y") +
  labs(y = expression(CPUE~(Count~m^-3))) +
  theme_bw()+
  theme(axis.text = element_text(size = 14), axis.title = element_text(size = 14), strip.text = element_text(size = 13), axis.text.x = element_text( size = 12, angle = 90), legend.position = "none") 
...
```{r}
### NMDS scores
order.scores <- as.data.frame(scores(nmds150.org))
species.scores <- as.data.frame(scores(nmds150.org, "species"))
species.scores$species <- rownames(species.scores)
# Need a category to merge scores with the rest of env data
row.names(order.scores) <- row.names(all.flip150.org)#change here to table with env variables and orgs
order.nmdsplot <- cbind(order.scores, all.flip150.org)#change here to table with env variables and orgs
########### STTD version
### NMDS scores
...
f_plotNMDS <- function(nmdsDf, nmdsDfSpecies, covariate, colors = cols, legendtitle=covariate) {
  # extends the plot a little to include species labels that are a bit stretched out; may need to modify based on your data
  limits_x = c(min(nmdsDfSpecies$NMDS1-1), max(nmdsDfSpecies$NMDS1+1))
  limits_y = c(min(nmdsDfSpecies$NMDS2-0.1), max(nmdsDfSpecies$NMDS2+0.1))
  
  # plot
  ggplot(nmdsDf, aes(NMDS1, NMDS2)) +
  geom_point(aes(color = .data[[covariate]],shape = .data[[covariate]]), size = 3, position = position_jitter(.1) ) +
  #ggrepel::geom_text_repel(data = nmdsDfSpecies, aes(label = species), max.overlaps = 20) + #try to avoid overlapping labels
  stat_ellipse(type = 't', size = 1, aes(color = .data[[covariate]],linetype = .data[[covariate]])) +
  annotate("text", x = min(order.nmdsplot$NMDS1) + 0.25, y = min(order.nmdsplot$NMDS2), label = paste('Stress = ', round(nmds150.org$stress,3))) + # Add stress to plot. change round to use NMDS results 
...
#*~use this for report~*#
(nmds_station <- f_plotNMDS(order.nmdsplot, species.scores,  "Station.Code", cols))
(nmds_WY <- f_plotNMDS(order.nmdsplot, species.scores, "WY", cols, "Water Year")) 
(nmds_inundation <- f_plotNMDS(order.nmdsplot2, species.scores2,  "Inundation", cols, "Inundation") )#use only STTD data here
nmds_season1 <- f_plotNMDS(order.nmdsplot, species.scores, "Season1", cols)
#*~use this for report~*#
(nmds_season2 <- f_plotNMDS(order.nmdsplot, species.scores, "Season2", cols2, "Season"))
nmds_season3_STTD <- f_plotNMDS(order.nmdsplot2,species.scores2, "Season3", cols2, "Season")
```
...
zoop.150.permanova = select(zoop150, Date, WY, Station.Code, Organism, Season2, CPUE)
...
all.flip.permanova <- spread(data=zoop.150.permanova, key = Organism, value= CPUE, fill = 0)

#extract columns with abundance info to create matrix for PERMANOVA
Zoop.permanova = all.flip.permanova[5:75]
...
zoop.150.STTD.permanova = select(zoop150.STTD, Date, WY, Station.Code, Organism, Inundation, Season2, CPUE)

#spread the data so that TaxonName are the column headers, CPUE is the cell value, and sitedate is the row names
#replace NAs with 0s
#Rosie's note: You can do this within your "spread" call by assigning "fill = 0"
all.flip.permanova.STTD <- spread(data=zoop.150.STTD.permanova, key = Organism, value= CPUE, fill = 0)

#extract columns with abundance info to create matrix for PERMANOVA
Zoop.permanova.STTD = all.flip.permanova.STTD[6:67]
