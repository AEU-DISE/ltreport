...
?jpeg
  ...
  WQ3<-ggplot(WQ2)+
    geom_line(aes(x=Date,y=Value, color=StationCode, linetype=StationCode))+
     scale_color_viridis(discrete=TRUE)+
     theme_Publication() + #or whatever ggtheme you want
     #labs(x = "Date", y = "") +
    facet_wrap(~Parameter, scales="free_y",nrow=5, strip.position = "left",labeller = as_labeller(c(DO = "DO (mg/L)", pH = "pH",SpCnd="SpCnd (mS/cm)", Turbidity="Turbidity (FNU)", WaterTemp="Water Temp (C)")))+
    theme(strip.background = element_rect(fill="White", color = "White"),
          strip.placement = "outside", axis.title.y = element_blank(), legend.position = "top")+
    
      scale_x_date(date_breaks = "3 months", date_labels= "%b-%Y")+
    theme(axis.text.x = element_text(size = 13, angle =90), axis.text.y = element_text(size = 13), strip.text = element_text(size = 13),legend.text = element_text(size = 13), legend.title = element_text(size = 13))

    WQ3
    
tiff(filename = "PhytoChloroOutputFigs/WQAll.tiff", res = 300, width = 10, height = 12, family = "sans", units = "in", compression = "lzw")
(WQ3)
dev.off()
```
