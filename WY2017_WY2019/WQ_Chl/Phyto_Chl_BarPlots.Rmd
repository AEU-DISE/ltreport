...
BSY<-ggplot()+                          ##Biovol by station each year
  ...
  scale_fill_viridis(discrete=TRUE)+
  theme_Publication()+
  labs(x="Water Year", y="Phytoplankton Density (μm3/ml)")

BSY
```

```{r}
write.csv(BioWY, here(root, "data_figures", "biovolume_bar.csv"), row.names = FALSE)
write.csv(ChlWY, here(root, "data_figures", "chlorophyll_bar.csv"), row.names = FALSE)
```
