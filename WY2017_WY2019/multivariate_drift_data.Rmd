...
df.nmdsplot <- cbind(nmds.scores, speciesPredictorMatrix)
  ...
  annotate("text", x = min(df.nmdsplot$NMDS1) + 0.25, y = min(df.nmdsplot$NMDS2), label = paste('Stress = ', round(nmdsResult$stress,3))) + # Add stress to plot
...
(nmds_station <- f_plotNMDS(nmdsOrder, Order_w2,  "Station", cols))
(nmds_WY <- f_plotNMDS(nmdsOrder, Order_w2,  "fWY", cols))
...
(nmds_season3 <- f_plotNMDS(nmdsOrder, Order_w2,  "Season3", cols2))
